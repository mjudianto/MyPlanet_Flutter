...
class _ElearningCoursePageState extends State<ElearningCoursePage> {
  ...
  Map<String, Future<List<dynamic>>> moduleDataFutures =
      {}; // Store lessons and tests data
  ...
  void initState() {
    ...
    _singleCourseFuture =
        elearningCoursePageController.fetchSingleCourseDetail();
    _courseModulesFuture = elearningCoursePageController.fetchCourseModules();

    _videoPlayerController =
        VideoPlayerController.networkUrl(Uri.parse(lessonVideoUrl))
          ..initialize().then((value) => setState(() {}));
    ...
    _courseModulesFuture?.then((modules) {
      for (var module in modules.data!) {
        ...
        final moduleLessonsFuture = elearningCoursePageController
            .fetchModuleLessons(module.elearningModuleId.toString());
        final moduleTestsFuture = elearningCoursePageController
            .fetchModuleTests(module.elearningModuleId.toString());

        moduleDataFutures[module.elearningModuleId.toString()] =
            Future.wait([moduleLessonsFuture, moduleTestsFuture]);
      }
    });
  }
          ...
          child: Column(
            children: [
              ...
              Flexible(
                child: Padding(
                  padding:
                      const EdgeInsets.only(left: 20.0, right: 20, top: 20.0),
                        ...
                        if (true) {
                          return SingleChildScrollView(
                            child: Column(
                              ...
                              final moduleDataFuture = moduleDataFutures[
                                  module.elearningModuleId.toString()];
                              bool lastModule = false;

                              if (modules.last.elearningModuleId ==
                                  module.elearningModuleId) {
                                lastModule = true;
                              }

                              return Padding(
                                padding: EdgeInsets.only(
                                    bottom: lastModule ? 32 : 16),
                                child: Container(
                                  decoration: BoxDecoration(
                                      color: whiteColor,
                                      borderRadius: BorderRadius.circular(15),
                                      border: Border.all(
                                          color: blackColor, width: 0.2)),
                                  child: Column(
                                    children: [
                                      ExpansionTile(
...
class StartPostTestConfirmationModal extends StatefulWidget {
  ...
  StartPostTestConfirmationModal(
      {super.key, required this.lesson, this.future});

  @override
  State<StartPostTestConfirmationModal> createState() =>
      _StartPostTestConfirmationModalState();
}
